...
class Game(object):
    ...
    def _assemble_state(self):
        ...
        def _get_technical_indicators(bars):
            ...
            # Defensive: Ensure input is a float64 numpy array, not object dtype
            close = pd.to_numeric(bars['close'], errors='coerce').values
            high = pd.to_numeric(bars['high'], errors='coerce').values
            low = pd.to_numeric(bars['low'], errors='coerce').values
            
            # print("----DEBUG BEFORE SMA(1)----")
            # print("type(close):", type(close))
            # print("len(close):", len(close))
            # print("close.dtype:", close.dtype)
            # print("All NaN?:", close.isnull().all())
            # print("Any NaN?:", close.isnull().any())
            # print("Any Inf?:", np.isinf(close).any())
            # print("close (head):", close.head())
            # print("close (tail):", close.tail())
            # print("close.values:", close.values)
            # print("Unique values:", close.unique())
            # print("--------------------------")
            # # Check for valid input length for the deepest indicator (99 bars)
            # if len(close) < 99 or close.isnull().any() or high.isnull().any() or low.isnull().any():
            #     print("Insufficient data for indicators, filling with np.nan")
            #     return np.full(16, np.nan) # 16 = number of features your function returns

            # Now safe to compute
            try:
                sma_values = [talib.SMA(close, timeperiod=1)[-1-shift] for shift in range(5)]
            except Exception as e:
                print("TA-Lib SMA(1) error:", e)
                sma_values = [np.nan]*5
            ...
            try:
                ...
                print("TA-Lib SMA(50) error:", e)
                sma50 = np.nan
            tech_ind = np.append(tech_ind, sma50)
            try:
                cci1 = talib.CCI(high, low, close, timeperiod=20)
                cci1_sma = talib.SMA(cci1, timeperiod=20)[-1]
            except Exception as e:
                print("TA-Lib CCI(20) or SMA error:", e)
                cci1 = np.full(len(close), np.nan)
                cci1_sma = np.nan
            tech_ind = np.append(tech_ind, cci1[-1]); tech_ind = np.append(tech_ind, cci1_sma)
            try:
                cci2 = talib.CCI(high, low, close, timeperiod=50)
                cci2_sma = talib.SMA(cci2, timeperiod=50)[-1]
            except Exception as e:
                print("TA-Lib CCI(50) or SMA error:", e)
                cci2 = np.full(len(close), np.nan)
                cci2_sma = np.nan
            tech_ind = np.append(tech_ind, cci2[-1]); tech_ind = np.append(tech_ind, cci2_sma)
            # The rest: ribbon_signal, cci_signal
    ...
    def step(self, action):
        reward, is_over = self.act(action)
        terminated = is_over
        truncated = False
        if not is_over:
            if self.curr_idx < len(self.bars30m) - 1: self.curr_idx += 1
            else: terminated = True; self.is_over = True
        self._assemble_state()
    ...
    class TrainingCallback(EvalCallback):
        ...
        def _on_step(self):
            ...
            if self.locals.get('dones', False):
                ...
                self.losses.append(self.locals.get('loss', 0))
                
                # Plot training curves every 1000 steps

...
class Game(object):
    def __init__(self, bars30m, bars1d, bars4h, reward_function, lkbk=20, init_idx=None):
        ...
        self.observation_space = spaces.Box(low=-1e6, high=1e6, shape=(len(self.state),), dtype=np.float32)
...
class GameGymWrapper(gym.Env):
    def __init__(self, game_instance):
        ...
        self.action_space = spaces.Discrete(3)
        self.observation_space = spaces.Box(low=-1e6, high=1e6, shape=(len(self.game.get_state()),), dtype=np.float32)
